...
class inventory_wizard(models.TransientModel):
    ...
    period_length = fields.Integer('Period Length (Months)', default=12, required="1")
    ...
    def get_aging_detail(self):
        ...
        for i in range(7)[::-1]:
            stop = start - relativedelta(months=period_length)
            res[str(i)] = {
                'name': (i != 0 and (str((7 - i) * period_length)) or (
                        '+' + str(6 * period_length))),
                'value': 'Values',
                'stop': start.strftime('%Y-%m-%d'),
                'start': (i != 0 and stop.strftime('%Y-%m-%d') or False),
            }
            start = stop - relativedelta(months=0)
        return res
    ...
    def print_excel(self):
        ...
        worksheet = workbook.add_sheet('Stock Inventory Aging', cell_overwrite_ok=True)
